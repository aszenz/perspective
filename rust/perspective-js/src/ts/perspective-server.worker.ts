...
import { PerspectiveSession } from "./engine.js";
import { compile_perspective } from "./emscripten_api.ts";
import { PerspectiveServer } from "./engine.ts";
...
self.addEventListener("message", async (msg) => {
    if (msg.data.cmd === "init") {
        ...
        session = server.make_session((resp) => {
        ...
        session.handle_message(new Uint8Array(msg.data));
    }
});
