...
// import type perspective from "../../dist/pkg/web/perspective-server.d.ts";
...
export class PerspectiveServer {
    clients: Map<number, (buffer: Uint8Array) => void>;
    id_gen: number;
     ...
     */
    make_session(callback: (buffer: Uint8Array) => void): PerspectiveSession {
...
export class PerspectiveSession {
    ...
    handle_message(view: Uint8Array) {
        ...
        decode_api_responses(this.mod, ptr, (msg: ApiResponse) => {
            this.client_map.get(msg.client_id)!(msg.data);
        });
    }

    poll() {
        const polled = this.mod._js_poll(this.server);
        decode_api_responses(this.mod, polled, (msg: ApiResponse) => {
            this.client_map.get(msg.client_id)!(msg.data);
        });
    }
 ...
 */
function decode_api_responses(
    core: EmscriptenApi,
    ptr: number,
    callback: (_: ApiResponse) => void
) {
