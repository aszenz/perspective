...
use perspective_client::{
    assert_table_api, assert_view_api, clone, Client, ClientError, OnUpdateArgs, OnUpdateMode,
...
fn is_coroutine(py: Python, obj: &PyAny) -> PyResult<bool> {
    let coroutine_type = py.import("types")?.getattr("CoroutineType")?;
    obj.is_instance(coroutine_type)
}
